...
public class ExpressionAndCteTests
{
  ...
  ""tables"": {
    ...
    {
        var sql = "select id from users where id = @id\n" +
            new string(' ', Extrode.JauntyQ.SqlParser.SqlTokenizer.MaxInputLength);
        var (result, _) = Run(sql, "db/Users/GetById.sql");
    ...
    public void InputAtExactMaxLength_NoJNT1003()
    {
        ...
        var sql = baseSql + new string(' ', Extrode.JauntyQ.SqlParser.SqlTokenizer.MaxInputLength - baseSql.Length);
        Assert.Equal(Extrode.JauntyQ.SqlParser.SqlTokenizer.MaxInputLength, sql.Length);
