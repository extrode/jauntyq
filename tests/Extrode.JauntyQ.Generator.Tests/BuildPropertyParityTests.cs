...
public class BuildPropertyParityTests
{
    ...
    {
        string source = ReadRepoFile("src", "Extrode.JauntyQ.Generator", "JauntyQGenerator.cs");
    ...
    {
        string xml = ReadRepoFile("src", "Extrode.JauntyQ.Generator", folder, "Extrode.JauntyQ.Generator.props");
        var names = new SortedSet<string>(StringComparer.Ordinal);
    ...
    public void ThePackagedPropsFileName_MatchesThePackageId()
    {
        ...
        string csproj = ReadRepoFile("src", "Extrode.JauntyQ.Generator", "Extrode.JauntyQ.Generator.csproj");
        var m = Regex.Match(csproj, @"<PackageId>([^<]+)</PackageId>");
        string packageId = m.Success ? m.Groups[1].Value.Trim() : "Extrode.JauntyQ.Generator";

        foreach (string folder in new[] { "build", "buildTransitive" })
            Assert.True(
                File.Exists(Path.Combine(RepoRoot(), "src", "Extrode.JauntyQ.Generator", folder, packageId + ".props")),
