...
/// Extrode.JauntyQ.Cli.Tests pins five of the dialect-name sites and records three it
...
public class CodeEmitterDialectParityTests
{
    ...
    private static string SourcePath(string file) =>
        Path.Combine(RepoRoot(), "src", "Extrode.JauntyQ.Generator", file);
    ...
    {
        Assert.True(File.Exists(SourcePath(file)), $"{file}: not found under src/Extrode.JauntyQ.Generator");
