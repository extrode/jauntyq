...
internal static class FunctionRoundTrip
{
    ...
    internal static async Task<Assembly> GenerateAndCompile(
        contract::Extrode.JauntyQ.Schema.DatabaseSchema schema, string dialect)
    {
    ...
    internal static MethodInfo StaticFn(Assembly asm, string name, int argCount)
        => asm.GetType("Extrode.JauntyQ.Generated.JauntyDb+FunctionAccessor")!
...
public class PostgresFunctionRoundTripTests : IClassFixture<PostgresFunctionRoundTripFixture>
{
    ...
    public async Task AFunctionOutsideTheSearchPathIsReachedOnlyByItsQualifiedName()
    {
        ...
        var schema = await new contract::Extrode.JauntyQ.Schema.Extraction.PostgresExtractor("reporting")
            .ExtractAsync(_fx.ConnectionString);
    ...
    public async Task ADomainTypedColumnRecordsItsProvenance_AndStillMapsToTheBaseType()
    {
        ...
        var asm = await FunctionRoundTrip.GenerateAndCompile(schema, "postgres");
        var row = asm.GetType("Extrode.JauntyQ.Generated.PeopleRow")!;
...
public class SqlServerFunctionRoundTripTests : IClassFixture<SqlServerFunctionRoundTripFixture>
{
    ...
    private async Task<Assembly> Build()
    {
        var schema = await new contract::Extrode.JauntyQ.Schema.Extraction.SqlServerExtractor()
            .ExtractAsync(_fx.ConnectionString);
    ...
    public async Task AFunctionOutsideTheDefaultSchemaIsReachedOnlyByItsQualifiedName()
    {
        ...
        var schema = await new contract::Extrode.JauntyQ.Schema.Extraction.SqlServerExtractor("reporting")
            .ExtractAsync(_fx.ConnectionString);
        Assert.Equal("reporting", schema.Functions.Values.Single(f => f.Name == "calc_tax").Schema);
