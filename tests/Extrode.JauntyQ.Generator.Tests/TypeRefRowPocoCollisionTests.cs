...
/// top-level classes in <c>namespace Extrode.JauntyQ.Generated</c>, derived from the
...
public class TypeRefRowPocoCollisionTests
{
  ...
  ""tables"": {
    ...
    public void TypeRef_RowPocoCollision_ReturnsGlobalQualified()
    {
        ...
        var schema = new Extrode.JauntyQ.Schema.DatabaseSchema();
        schema.Tables["date_times"] = new Extrode.JauntyQ.Schema.TableSchema
        {
            Name = "date_times",
            Columns = new Dictionary<string, Extrode.JauntyQ.Schema.ColumnSchema>
            {
                ["id"] = new Extrode.JauntyQ.Schema.ColumnSchema { Name = "id", DbType = "int", IsPrimaryKey = true }
            }
        };
