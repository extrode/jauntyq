...
public class ToolPackagingTests
{
    ...
    private static string CliCsproj() =>
        File.ReadAllText(Path.Combine(RepoRoot(), "src", "JauntyQ.Cli", "JauntyQ.Cli.csproj"));
    ...
    [InlineData("PackageId", "JauntyQ.Cli")]
    ...
    public void Cli_ReferencesOnlyTheCoreLibrary()
    {
        ...
        Assert.Equal(["JauntyQ.Cli.Core.csproj"], references);
    }
    ...
    public void PrintedUsage_UsesTheInstalledCommandName()
    {
        string program = File.ReadAllText(
            Path.Combine(RepoRoot(), "src", "JauntyQ.Cli.Core", "SchemaPullVerb.cs"));
