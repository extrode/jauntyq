...
/// (Northwind, Extrode.JauntyQ.Schema.Extraction.Tests, Postgres.Tests, MySql.Tests) skipped cleanly in the same run,
...
public class FixtureContainerBuildSiteTests
{
    ...
    public void EveryDockerBackedTestClass_GatesWithSkippableFact()
    {
        ...
        // fixtures in Extrode.JauntyQ.Schema.Extraction.Tests hold no container of their own -- they draw a database
    ...
    // EngineContainers.cs, and everything else in tests/Extrode.JauntyQ.Schema.Extraction.Tests goes through it.
    ...
    {
        string root = RepoRoot();
        string projectDir = Path.Combine(root, "tests", "Extrode.JauntyQ.Schema.Extraction.Tests");
        List<string> offenders = new();
        int scanned = 0;
        ...
        Assert.True(
            offenders.Count == 0,
            "Fixtures in Extrode.JauntyQ.Schema.Extraction.Tests must draw a per-fixture database from the shared "
