...
public class FavoriteTests
{
    ...
    public FavoriteTests(ConduitPostgresFixture fx)
    {
        _fx = fx;
    ...
    // Favorites/Insert.sql - expect a raw PostgresException.
    [SkippableFact]
    public void Favorite_Duplicate_ThrowsOnPrimaryKeyConflict()
    {
        Skip.IfNot(_fx.Available, _fx.SkipReason);

        Assert.Throws<PostgresException>(() => _repo.Favorite(userId: 1, articleId: 1));
    }
}
