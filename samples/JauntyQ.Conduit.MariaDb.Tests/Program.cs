...
using JauntyQ.Conduit.MariaDb.Tests.Auth;
...
builder.Services.AddScoped<MySqlConnection>(sp =>
{
    string connectionString = sp.GetRequiredService<IConfiguration>()["ConnectionString"]
        ?? "Server=localhost;Database=conduit;Uid=root";
    var conn = new MySqlConnection(connectionString);
    conn.Open();
    return conn;
});
builder.Services.AddScoped(sp => new JauntyDb(sp.GetRequiredService<MySqlConnection>()));
