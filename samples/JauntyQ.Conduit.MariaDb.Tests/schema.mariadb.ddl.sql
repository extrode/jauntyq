-- DDL-only counterpart to schema.mariadb.sql, for fixtures that must start
-- from an empty database. schema.mariadb.sql's seed INSERTs (jane/bob/carol/
-- dave, sample articles, etc.) are fixture data for the direct-repository
-- tests (ConduitMariaDbFixture) -- HTTP-level black-box tests instead create
...
--
-- See schema.mariadb.sql for the notes on VARCHAR date-string columns, the
-- out-of-line FOREIGN KEY authoring (inline REFERENCES is silently dropped by
-- MySQL/MariaDB), and the `follows` ON DELETE CASCADE (kept as-is on MariaDB,
