...
/// Part 2, run against real SQLite. Expected values were captured by running
/// each query's literal SQL against the identical curated dataset via
/// sqlite3, then cross-checked row-for-row against the Postgres port
/// (Extrode.JauntyQ.Sakila.Postgres.Tests.SakilaQueriesTests) before being hardcoded
/// here — see the test log for the one real divergence found
/// (month-boundary timezone handling in query 4) and how it was fixed.
/// </summary>
public class SakilaQueriesTests : IClassFixture<SakilaSqliteFixture>
{
    private readonly SakilaSqliteFixture _fx;
    public SakilaQueriesTests(SakilaSqliteFixture fx) => _fx = fx;
