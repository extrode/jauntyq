...
namespace Microsoft.eShopWeb.Sqlite.Tests;
...
/// writes" and documents SQLite's default locking behavior as a finding
/// rather than a pass/fail assertion.
/// </summary>
public class ConcurrencyTests : IClassFixture<EShopOnWebSqliteFixture>
{
    private readonly EShopOnWebSqliteFixture _fx;

    public ConcurrencyTests(EShopOnWebSqliteFixture fx) => _fx = fx;

    private (SqliteConnection Conn, JauntyDb Db) OpenIndependentConnection()
    {
        var conn = new SqliteConnection(_fx.ConnectionString);
        conn.Open();
    ...
    public async Task ConcurrentUpdates_UnderTransactions_DoNotDeadlock()
    {
        ...
        // SQLite's default rollback-journal locking serializes ALL writers
        // at the whole-database level (not per-row like Postgres) - the
        // second writer's transaction blocks until the first commits, via
        // Microsoft.Data.Sqlite's default busy-timeout wait rather than an
        // immediate SQLITE_BUSY error. Documented as a finding in
        // the test log, not asserted here since blocking
