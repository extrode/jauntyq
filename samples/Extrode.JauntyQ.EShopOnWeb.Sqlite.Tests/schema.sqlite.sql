...
--
-- order_date declared DATETIME (NUMERIC affinity, so SQLite stores the
-- ISO-8601 text verbatim rather than coercing it) so the generator infers
-- a C# DateTime column, keeping OrderRepository dialect-agnostic. UTC
-- instant only, per Part 1 scope decision -- SQLite has no native
-- timestamp/offset type.
-- price/unit_price use NUMERIC affinity; SQLite's NUMERIC/DECIMAL columns
-- are IEEE-754 double under the hood (known boundary, logged in
-- docs/torture-test-log.md) -- tests use tolerance-based decimal asserts.
