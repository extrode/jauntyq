...
dotnet run -c Release --project benchmarks/JauntyQ.Benchmarks

# One group
dotnet run -c Release --project benchmarks/JauntyQ.Benchmarks -- --filter '*ReadBenchmarks*'

# Fast smoke run (3 iterations, not for publishable numbers)
dotnet run -c Release --project benchmarks/JauntyQ.Benchmarks -- --filter '*' --job short
```
