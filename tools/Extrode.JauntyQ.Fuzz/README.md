...
```sh
dotnet publish tools/Extrode.JauntyQ.Fuzz -c Release -o out/fuzz
dotnet tool install --global SharpFuzz.CommandLine
sharpfuzz out/fuzz/Extrode.JauntyQ.SqlParser.dll
...
./libfuzzer-dotnet -max_total_time=600 -artifact_prefix=out/fuzz-findings/ \
  --target_path="$(command -v dotnet)" --target_arg=out/fuzz/Extrode.JauntyQ.Fuzz.dll \
  out/corpus tools/Extrode.JauntyQ.Fuzz/corpus
```
...
`tools/Extrode.JauntyQ.Fuzz/corpus` keeps its 18 committed seeds untouched. Nightly caches `out/corpus`
...
./libfuzzer-dotnet -merge=1 \
  --target_path="$(command -v dotnet)" --target_arg=out/fuzz/Extrode.JauntyQ.Fuzz.dll \
  out/corpus-min out/corpus tools/Extrode.JauntyQ.Fuzz/corpus
```
...
That fallback is a useful single-input replay — `dotnet out/fuzz/Extrode.JauntyQ.Fuzz.dll corpus/001-select.sql`
