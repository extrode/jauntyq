...
public partial class JauntyQGenerator : IIncrementalGenerator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        int tooLargeIndex = tokens.FindIndex(t => t.Type == Extrode.JauntyQ.SqlParser.Tokens.TokenType.TooLarge);
        if (tooLargeIndex >= 0)
        {
        ...
        // parsing.
        int tooDeepIndex = tokens.FindIndex(t => t.Type == Extrode.JauntyQ.SqlParser.Tokens.TokenType.TooDeep);
        if (tooDeepIndex >= 0)
        {
        ...
        int unterminatedIndex = tokens.FindIndex(t => t.Type == Extrode.JauntyQ.SqlParser.Tokens.TokenType.Unterminated);
        ...
        int unknownIndex = tokens.FindIndex(t => t.Type == Extrode.JauntyQ.SqlParser.Tokens.TokenType.Unknown);
        if (unknownIndex >= 0)
        {
