...
{
    /// <summary>
    /// The tail appended to a JNT8004 raised on a SYNTHESIZED (auto-CRUD)
    /// query. The base message ends "Add an index or filter on an indexed
    /// column" -- advice a consumer cannot take when the schema is upstream-
    /// owned (canonical sakila/pagila/northwind), and the -- @allow-unindexed
    /// escape reads as unavailable because there is no file to put it in.
    ///
    /// There is one: a hand-written .sql file claims its entity.method slot
    /// before synthesis (the claimedMethods.Add guard in the synth loop below,
    /// against the set seeded from the file summaries), so writing the file
    /// below overrides the generated method and carries the directive with it.
    /// That already worked; nothing pointed at it. Naming the exact path and
    /// pasting the SQL is the whole fix.
    ///
    /// Takes cleanedSql -- the caller's DirectiveParser output -- rather than
    /// synth.Sql, and that is load-bearing rather than incidental. Two
    /// synthetics already carry a leading directive line (GetById's "-- @first",
    /// AutoCrud.cs:144; Insert's "-- @identity", :161). Neither can raise
    /// JNT8004 today -- GetById's PK filter short-circuits at
    /// IsColumnIndexSupported and Insert has no filter at all -- but nothing
    /// enforces that, and flattening a string whose first line is "--" to a
    /// single line comments out the entire statement. A consumer pasting that
    /// would create a file holding only comments, which is the JNT3001
    /// directive-only trap: the slot is claimed, the SQL is empty, and the
    /// method disappears from the generated API. cleanedSql has the directive
    /// lines already stripped, so the flatten is safe for any shape that ever
    /// reaches here. For the GetBy&lt;Fk&gt; family the two are byte-identical.
    ///
    /// commonPrefix is the grandparent root of the consumer's existing .sql
    /// files (Part6.cs:20), i.e. the folder whose subdirectories become entity
    /// names -- so `{prefix}/{Entity}/{Method}.sql` is the path that yields
    /// this exact entity and method back through ExtractEntityName. It is ""
    /// when the consumer has no .sql files at all, which is the common case
    /// for a pure auto-CRUD project; describe the rule rather than invent a
    /// path, because a guessed root that does not match the consumer's
    /// AdditionalFiles glob would be worse than no path.
    /// </summary>
    private static string SyntheticOverrideHint(
        AutoCrud.SyntheticQuery synth, string cleanedSql, string commonPrefix)
    {
        ...
        return $" '{synth.EntityName}.{synth.MethodName}' is generated by auto-CRUD, so there is no file " +
               $"carrying it to annotate. To accept this scan deliberately, create {where} containing " +
        ...
        string? acceptJson = null)
    {
        ...
        // file compiled into Extrode.JauntyQ.Generated -- and the always-emitted
