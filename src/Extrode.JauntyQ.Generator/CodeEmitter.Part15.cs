...
public static partial class CodeEmitter
{
    ...
    private static bool IsUsableSqlName(string name, string? dialect)
    {
        if (!IdentifierGuard.IsValidIdentifier(name))
            return false;
        if (Extrode.JauntyQ.Analysis.DialectReservedWords.IsReservedInDialect(
                name, dialect, Extrode.JauntyQ.Analysis.SqlIdentifierPosition.Object))
            return false;
        if (Extrode.JauntyQ.Analysis.DialectReservedWords.RequiresQuotingForCase(name, dialect))
            return false;
        return true;
    }
