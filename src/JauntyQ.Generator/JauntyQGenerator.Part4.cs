...
public partial class JauntyQGenerator : IIncrementalGenerator
{
    ...
    /// before synthesis (Part4.cs:327 against the set seeded at :119-122), so
    /// writing the file below overrides the generated method and carries the
    /// directive with it. That already worked; nothing pointed at it. Naming
    /// the exact path and pasting the SQL is the whole fix.
    ///
    ...
    /// </summary>
    private static string SyntheticOverrideHint(AutoCrud.SyntheticQuery synth, string commonPrefix)
    {
        // Single-line: build output is line-oriented and the synthesized FK
        // loader is one SELECT, so it stays paste-able.
        string oneLineSql = synth.Sql.Replace("\r\n", " ").Replace('\n', ' ').Replace('\r', ' ');
        ...
        return $" '{synth.EntityName}.{synth.MethodName}' is generated by auto-CRUD, so there is no file " +
               ...
               "overrides the generated method of the same name, and the directive then suppresses this " +
               "warning for that query only. To drop the generated loader instead, set " +
                ...
                foreach (var error in errors)
                {
                    if (error.Severity == ValidationSeverity.Error)
                        ...
                        context.ReportDiagnostic(Diagnostic.Create(
                            ...
                            error.Message + SyntheticOverrideHint(synth, commonPrefix)));
                    else
