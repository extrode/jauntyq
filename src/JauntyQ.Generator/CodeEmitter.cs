...
""");
    ...
    public static string EmitRowPoco(string rowTypeName, TableSchema tableSchema)
    {
        ...
        foreach (var col in tableSchema.Columns.Values)
        {
            ...
            sb.AppendLine($"        public {modifier}{csharpType} {DialectMapper.ToPascalCase(col.Name)} {{ get; set; }}");
        }
        sb.AppendLine();
        ...
        bool hasUpsert)
    {
        ...
        string Args(System.Collections.Generic.List<ColumnSchema> cols)
            => string.Join(", ", cols.Select(c => $"row.{DialectMapper.ToPascalCase(c.Name)}"));
        ...
        if (hasInsert && insertCols.Count > 0)
        {
            if (insertReturnsId)
            {
                string idProp = DialectMapper.ToPascalCase(identityCol!.Name);
