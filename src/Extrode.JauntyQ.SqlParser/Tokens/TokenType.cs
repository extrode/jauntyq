namespace Extrode.JauntyQ.SqlParser.Tokens;
...
public enum TokenType
{
    ...
    /// quoted identifier (<c>[Name</c>, <c>"Name</c>, or <c>`Name</c> with no
    /// closing delimiter), or string literal (<c>'text</c> with no closing
    /// <c>'</c>) that ran to end-of-input. The tokenizer stops at this point
    /// instead of silently treating the rest of the file as comment/
    /// identifier/literal text; <see cref="Token.Value"/> holds a short
    /// description of what wasn't closed.
    /// </summary>
    Unterminated,
